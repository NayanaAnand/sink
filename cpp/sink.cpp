...
PREPARE_LOGGING(sink_i)

sink_i::sink_i(const char *uuid, const char *label) :
    sink_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void sink_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &sink_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &sink_i::<callback method>)
        in the constructor.
        ...
        sink_i::sink_i(const char *uuid, const char *label) :
            sink_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &sink_i::scaleChanged);
            addPropertyListener(status, this, &sink_i::statusChanged);
        }

        void sink_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void sink_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
